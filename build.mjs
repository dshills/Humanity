...
let bundle = '';
// Data files numbered up to CURATED_LAST are written by hand; the rest are generated by scripts/import-*.mjs.
// The page is told where the hand-written events end so it can prefer them when choosing an event's neighbours.
const CURATED_LAST = 8;
const dataNumber = (name) => { const m = /^data\/(\d+)-/.exec(name); return m ? Number(m[1]) : null; };
let curatedMarked = false;
for (const { name, text } of sources) {
  const n = dataNumber(name);
  if (!curatedMarked && (n === null ? bundle.includes(`src/${DATA_DIR}/`) : n > CURATED_LAST)) {
    bundle += `/* ======== end of the hand-curated events ======== */\nHT.curatedCount = (HT.events || []).length;\n\n`;
    curatedMarked = true;
  }
  bundle += banner(name);
