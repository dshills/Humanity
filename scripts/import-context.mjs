...
// Chandler's table stops in 1975, which would leave the ribbon blank for the last half-century. Its final column
// already has Tokyo ahead of New York, and Tokyo has been the largest urban agglomeration in every revision of the
// UN World Urbanization Prospects since (37.4 million in the 2018 revision), so the ribbon is carried to the
// present with that one figure. The page draws the last segment through to today.
const lastWinner = winners[winners.length - 1];
if (lastWinner && lastWinner.y === 1975 && lastWinner.name === 'Tokyo') cityOut.push([1975, 2025, 'Tokyo', 37400000]);
else console.warn(`city ribbon not extended: the 1975 leader is ${lastWinner && lastWinner.name}`);
...
const out = `(function (root) {
  ...
  //      estimates from Chandler's table, digitised by Reba, Reitsma & Seto 2016 (figshare, CC BY 4.0); the final
  //      Tokyo segment after 1975 is from the UN World Urbanization Prospects 2018.
  HT.context = {
