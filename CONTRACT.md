...
src/app/NN-<topic>.js      HT.app — rendering, interaction, URL state: parts joined by the build inside ONE closure
...
`time.js, tiers.js, ticks.js, layout.js, core.js, earth.js, context.js, map.js, geo.js, objects.js, tours.js, data/*.js (sorted by filename), app/*.js (sorted by filename, wrapped together in one IIFE)`.
The parts under `src/app/` are the one exception to §1: each is a plain script of top-level declarations that shares scope with the others, must parse on its own as strict code (the build checks), and is never loaded by itself; shared `const`/`let` state is declared in `00-state.js`.
...
The final `<script>` ends with `HT.app.init();` — the app parts must NOT auto-run on load, and
must not use `DOMContentLoaded` (the script tag is placed at the end of `<body>`).

## 1. Script wrapper (every src/*.js file; the parts under src/app/ share one, added by the build)
...
Measuring: `measureFrom` (event index) is serialised as `from=<slug>` (validated `[a-z0-9-]{1,80}`); `#panel-measure` (`#measure-gap`, `#measure-lines`, `#measure-start`, `#measure-stop`), `#measure-chip` and `.g-span` (`.measure-anchor`, `.measure-span`, `.measure-label`) show it. `HT.core.gapBetween(a, b)` is the distance between nearest ends (0 on overlap), `fmtDuration(years)`, and `measureFacts(list, i, j, now, curated)` → `{ gap, earlier, later, lines }` where landmarks are curated point events of tier ≤ 2.
Header widths: `#layer-group` wraps `#btn-earth, #btn-reigns, #btn-lives, #btn-legend` and is `display: contents` except from 641 to 1180 px, where it is a popover toggled by `#btn-layers` (`.open`, closed by Escape, an outside click, choosing Legend, or growing wide again); `#btn-home` hides under 900 px; from 641 to 820 px the header wraps like the phone header; the HUD drops its Earth row under 1040 px and rises above the dock under 860 px.
